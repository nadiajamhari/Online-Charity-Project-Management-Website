...
<head>
    ...
    <title>Committee Members: <?php echo $chname;?></title>
</head>
...
<body>
    ...
    <div id="main">
        ...
        <main class="container">
            ...
            <h2><?php echo $chname;?></h2>

            <!--Table Sebenar-->

            <div class="card">
                <h3 class="card-header text-center font-weight-bold text-uppercase py-4">LIST OF COMMITTEE MEMBERS</h3>
                <div class="card-body">
                <div class="pull-right">
                        <a href="addNewCommittee.php">
                    ...
                    <div class="table-editable">
                        <table class="table table-bordered table-responsive-md table-striped">
                            ...
                            <tbody>
                            <?php
                            // tambah where id tu nanti
                                $sql = "SELECT * FROM committee where userID=$userID";
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        $commiteeID = $row['committeeID'];
                                        $name = $row['name'];
                                        $email = $row['email'];
                                        echo "<tr><td>".$commiteeID." </td>";
                                        echo "<td>". $name."</td>";
                                        echo "<td>".$email."</td>";
                                        echo "<td><a href='viewCommittee.php?id=$commiteeID' alt='view'><button class='btn-default a-btn-slide-text'><span class='glyphicon glyphicon-folder-open'
                                        aria-hidden='true'></span></a>
                                        </button>&nbsp;
                                        <button type='button' class='btn-danger a-btn-slide-text' 
                                        data-deleteid=".$commiteeID."><span class='glyphicon glyphicon-trash' 
